...
sealed class UdfToolsException(message: String) : Exception(message) {
    class CannotOpen(name: String) : UdfToolsException("$name dosyası açılamadı.")
    class InvalidDocument(name: String) : UdfToolsException("$name geçerli bir UDF belgesi değil.")
    class NoFiles : UdfToolsException("En az iki UDF dosyası seçmelisiniz.")
    class PasswordRequired : UdfToolsException("Parola gerekli.")
    class InvalidPassword : UdfToolsException("Parola hatalı veya şifreli dosya bozuk.")
    class EncryptionFailed : UdfToolsException("UDF dosyası şifrelenemedi.")
    class WriteFailed : UdfToolsException("UDF dosyası kaydedilemedi.")
}
...
object UdfToolsService {
    ...
    fun mergeUdf(files: List<File>, context: Context): File {
        ...
        val merged = UdfEditDocument(
            ...
            blocks = models.flatMap { it.blocks },
            footers = first.footers,
        )
    ...
    fun compress(file: File, context: Context): UdfCompressionResult {
        ...
        val output = OutputPaths.outputFile(context, "Sikistirilmis_${file.nameWithoutExtension}", "udf")
        try {
    ...
    fun encrypt(file: File, password: String, context: Context): File {
        ...
        return try {
            ...
            val output = OutputPaths.outputFile(context, "Sifreli_${file.nameWithoutExtension}", "udfenc")
            output.writeBytes(outputBytes)
            output
    ...
    fun decrypt(file: File, password: String, context: Context): File {
        ...
        val minimumLength = magicBytes.size + SALT_BYTES + NONCE_BYTES + 16
        if (encrypted.size <= minimumLength || !encrypted.copyOfRange(0, magicBytes.size).contentEquals(magicBytes)) {
